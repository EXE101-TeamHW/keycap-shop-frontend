import { CustomRequest, ImageFile, StaffNotes, CustomerFeedback, RequestHistory } from '../types/customRequest'
...
export function saveRequest(
  request: Omit<CustomRequest, 'id' | 'createdAt' | 'updatedAt' | 'status' | 'resultImages' | 'staffNotes' | 'customerFeedback' | 'revisionCount' | 'history'>
): string {
  try {
    const requests = getAllRequests()
    ...
    try {
      ...
      if (storageError instanceof DOMException && 
        ...
        throw new Error('Storage limit reached. Please remove some old requests or reduce image sizes.')
      }
      throw storageError
    }
...
export function getStorageInfo(): {
  ...
  const imageCount = requests.reduce((count, req) => count + req.images.length, 0)
 ...
 */
export function getRequestsByStatus(status: CustomRequest['status']): CustomRequest[] {
  const requests = getAllRequests()
 ...
 */
export function getRequestsAwaitingApproval(customerEmail: string): CustomRequest[] {
  const requests = getAllRequests()
  return requests.filter(req => 
